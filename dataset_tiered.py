...
class dataset_tiered(object):
    ...
    def next_data(self, n_way, n_shot, n_query, num_unlabel=0, n_distractor=0):
        """
    ...
    def sample(self, n_way, n_shot, n_query=0, num_unlabel=0, n_distractor=0):
        """
            sample function used for reptile framework
        """
        selected_classes = np.random.permutation(self.n_classes)[:n_way+n_distractor]
        for class_idx, cls in enumerate(selected_classes):
            idxa = self.dict_index_label[cls]
            np.random.RandomState().shuffle(idxa)
            idxs = idxa[0:n_shot + n_query]
            for idx in idxs:
                yield (self.image_data[idx], class_idx)
...
def compress(path, output):
  with np.load(path, mmap_mode="r") as data:
    images = data["images"]
    array = []
    for ii in tqdm(six.moves.range(images.shape[0]), desc='compress'):
      im = images[ii]
